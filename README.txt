...
- For docker compose to run, please open the .env file and change the value of docker_id accordingly. Once the Kong GUI is running, please import the esd-kong-postgres.json file placed in this same directory (import the services followed by the routes). This will allow our API Gateway to function.

-We have implemented an API gateway which might lead to some cold start problems. Please be patient and refrain from double-clicking. It might take a while to load at first, but will become a lot faster later. We have also implemented an external RDS database but since it is based in Singapore, retrieving data is extremely quick. More info is given below.
